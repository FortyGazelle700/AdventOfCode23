...
fn get_hand_type_priority(hand: String) -> u8 {
  ...
  hand.chars().for_each(|char: char| {
    let matching_chars: bool = repeats.iter().filter(|set: &&(String, usize)| {set.0 == char.to_string()}).collect::<Vec<&(String,usize)>>().len() == 0;
  ...
  dbg!(repeats.clone());
  repeats.clone().into_iter().for_each(|repeat: (String,usize)| {
